...
pm_clone_usage() {
  ...
  if [[ -n "$long_usage" ]]; then
    ...
    printf "%s\n" "Examples:"
    printf "  pm clone git@github.com:alexis-moins/recipe.git -s tools -n recipe\n"
    echo
...
pm_template_usage() {
  ...
  printf "%s\n" "Commands:"
  printf "  %s   Set a template as global\n" "global"
...
pm_template_global_usage() {
  printf "pm template global - Set a template as global\n\n"
  printf "Alias: g\n"
  echo

  printf "%s\n" "Usage:"
  printf "  pm template global [TEMPLATE]\n"
  printf "  pm template global --help | -h\n"
  echo
  ...
  if [[ -n "$long_usage" ]]; then
    ...
    printf "%s\n" "Examples:"
    printf "  pm template global\n"
    printf "  pm template global react\n"
    echo
...
pm_backend_usage() {
  ...
  printf "%s\n" "Commands:"
  printf "  %s   Set a backend as global\n" "global"
...
pm_backend_global_usage() {
  printf "pm backend global - Set a backend as global\n\n"
  printf "Alias: g\n"
  echo

  printf "%s\n" "Usage:"
  printf "  pm backend global [BACKEND]\n"
  printf "  pm backend global --help | -h\n"
  echo
  ...
  if [[ -n "$long_usage" ]]; then
    ...
    printf "%s\n" "Examples:"
    printf "  pm backend global\n"
    printf "  pm backend global vscode\n"
    echo
...
pm_new_parse_requirements() {
    ...
    case "$key" in
      ...
      --space | -s)
        ...
        if [[ -n ${2+x} ]]; then
          ...
          printf "%s\n" "$(error)--space requires an argument: --space, -s SPACE" >&2
          exit 1
        fi
        ;;
      ...
      --name | -n)
        ...
        if [[ -n ${2+x} ]]; then
          ...
          printf "%s\n" "$(error)--name requires an argument: --name, -n NAME" >&2
          exit 1
        fi
        ;;
  ...
  if [[ -n ${args['--space']+x} ]] && [[ -z "${args[--name]:-}" ]]; then
    printf "%s\n" "$(error)--space flag requires --name" >&2
    exit 1
  fi

  if [[ -n ${args['--name']+x} ]] && [[ -z "${args[--space]:-}" ]]; then
    printf "%s\n" "$(error)--name flag requires --space" >&2
    exit 1
  fi
...
pm_clone_parse_requirements() {
  ...
  if [[ -z ${args['--space']+x} ]]; then
    printf "$(error)missing required flag: --space, -s SPACE\n" >&2
    exit 1
  fi
  if [[ -z ${args['--name']+x} ]]; then
    printf "$(error)missing required flag: --name, -n NAME\n" >&2
    exit 1
  fi
...
pm_space_add_parse_requirements() {
  ...
  if [[ -z ${args['space']+x} ]]; then
    printf "$(error)missing required argument: SPACE\n" >&2
...
pm_template_parse_requirements() {
  ...
  case $action in
    ...
    global | g)
      action="global"
      shift
      pm_template_global_parse_requirements "$@"
      shift $#
      ;;
...
pm_template_list_parse_requirements() {
    ...
    case "$key" in
      ...
      --only | -o)
        ...
        if [[ -n ${2+x} ]]; then
          ...
          printf "%s\n" "$(error)--only requires an argument: --only, -o GROUP" >&2
          exit 1
        fi
        ;;
  ...
  if [[ ${args['--only']:-} ]] && [[ ! ${args['--only']:-} =~ ^(user|default)$ ]]; then
    printf "%s\n" "$(error)--only must be one of: user, default" >&2
    exit 1
  fi
...
pm_backend_parse_requirements() {
  ...
  case $action in
    ...
    global | g)
      action="global"
      shift
      pm_backend_global_parse_requirements "$@"
      shift $#
      ;;
...
run() {
  ...
  case "$action" in
    ...
    "template global") pm_template_global_command ;;
    ...
    "backend global") pm_backend_global_command ;;
